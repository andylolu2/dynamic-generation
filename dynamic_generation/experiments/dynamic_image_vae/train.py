...
from dynamic_generation.models.image_vae import DynamicImageVae
from dynamic_generation.types import TrainState
from dynamic_generation.utils.distributions import deterministic_sample
    ...
    def evaluate(self):
        ...
        for item in self.eval_loader:
            ...
            if reconstruction is None:
                n = self.config.eval.reconstruct_samples
                pred, _ = out["X_hat"].sample_detailed(mode="deterministic")
                reconstruction = (x[:n], pred[:n])
...
if __name__ == "__main__":
    app.run(DynamicImageVaeTrainer.run)
